...
CHIP_ERROR ReadClient::SendReadRequest(NodeId aNodeId, Transport::AdminId aAdminId)
{
    ...
    {
        mpDelegate->HandlePathPrepareNeededCallBack(eventPathParamsList, aEventPathParamsListSize);
    }

    {
        ...
        msgBuf = System::PacketBufferHandle::New(kMaxSecureSduLength);
...
CHIP_ERROR ReadClient::ProcessReportData(System::PacketBufferHandle aPayload)
{
...

    {
        ...
        if (isEventListPresent && nullptr != mpDelegate && !moreChunkedMessages)
        {
            ...
            err = mpDelegate->HandleEventStreamReceivedCallBack(mpExchangeCtx, &reader);
            SuccessOrExit(err);
        }
    }
    ...
    {
        err = mpDelegate->HandleReportProcessedCallBack();
    }
